...
class MatchedRoute {
  ...
  constructor(route, matchResult, method) {
    this.#route = route;
  ...
  getHandler() {
    const handler = this.#route[this.#method];
    return handler ? handler.bind(this.#route) : null;
  }
...
class Router {
  ...
  resolve(path, method) {
    ...
    for (const { route, regex } of this.#compiledRoutes) {
      ...
      if (matchResult && route[method]) matched.push(new MatchedRoute(route, matchResult, method));
    }
